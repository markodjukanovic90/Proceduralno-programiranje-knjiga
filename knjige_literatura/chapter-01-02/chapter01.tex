...
Modul može sadržati različite definicije kao što su funkcije, klase i varijable, koje se mogu uvesti i koristiti u drugim Python skriptama ili modulima. Ovo olakšava organiziranje i ponovnu upotrebu k\^oda (eng.\ \emph{re-usability}) u različitim projektima.
...
\begin{itemize}
	\item name: Vraća naziv operativnog sistema (npr. `posix' za Linux i macOS, `nt' za Windows).
...
\end{minted}
\textit{Finally} blok: Za izvršavanje koda bez obzira na to da li je izuzetak aktiviran ili ne koristi se \textit{finally} kao u narednom primjeru:
\begin{minted}{python}
try:
