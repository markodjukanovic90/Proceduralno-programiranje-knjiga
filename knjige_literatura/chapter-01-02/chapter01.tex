...
PyPI (\textit{Python Package Index}) je centralni repozitorij paketa   koji su napisani u programskom jeziku Pajton. PyPI omogućuje programerima lako distribuiranje korisnički napisanih modula,  omogućavajući kako korišćenje paketa široj zajednici korisnika.
 
Nekoliko   se koraka izvršava ukoliko želimo postaviti   korisnički kreirani modul na PyPI. Na zvaničnoj PyPI veb stranici \footnote{https://pypi.org/account/register/},  potrebno je kreirati račun. Nakon što se prijavimo, potrebno je  generisati token za prijavu na PyPI. Token je sigurnosni ključ koji omogućuje da se prijavimo i objavimo/podignemo  paket (modul).  Na lokalnom računaru instalirajmo module  \texttt{setuptools} i \texttt{wheel}, u slučaju da nisu instalirani. To se možete učiniti pomoću \textit{pip} naredbe:
 \begin{minted}{python}
