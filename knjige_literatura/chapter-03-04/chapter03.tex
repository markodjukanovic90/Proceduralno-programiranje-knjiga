...
Kada je riječ o \textit{algoritamskim paradigmama}, one predstavljaju  pristupe ili metode za dizajniranje algoritama koji rješavaju   računske probleme. Postoji nekoliko algoritamskih paradigmi; svaka  dolazi sa svojim prednostima i slabostima. \\ 
...
Jedna od prvih algoritamska tehnika koja se može primijeniti u rješavanju  računarskih problema je tehnika grube sile. Ovo je algoritamska paradigma sa kojom se većina čitalaca susretala do sada, mada ne znajući da je riječ o baš toj tehnici rješavanja problema.
...
\begin{example}
	Odličan primjer algoritma grube sile je pronalaženje najbližeg para tačaka u višedimenzionalnom (realnom) prostoru. Neka su date $n$ tačaka u $\mathbb{R}^m$, tj. $S=\{ \textbf{x}_i \mid \textbf{x}_i \in  \mathbb{R}^m, i=1, \ldots, n\}$. Naći dvije tačke $\textbf{x}_i, \textbf{x}_j \in S$ koje su međusobno najbliže gledajući sve parove tačaka skupa $S$.
\end{example}
\begin{solution}
    Da bismo pronašli odgovor grubom silom,  izračunamo udaljenost između svakog  para tačaka skupa $S$, a zatim pratimo minimalnu pronađenu udaljenost. Verzija ovog algoritma je data narednim k\^odom.
...
Dakle, da bimo primijenili pohlepni pristup u rješavanju problema, potre\-bno je definisati: ($i$) skup komponenti rješenja; ($ii$) strukturu parcijalnog rješenja; ($iii$) pohlepnu funkciju koja računa kvalitet komponente koja može proširiti postojeće parcijalno rješenje.



Ovu paradigmu možemo koristiti kada je potrebno vratiti rješenje razumnog kvaliteta   u relativno kratkom vremenskom intervalu, implementirajuči pristup relativno lako. Ovo je često jedna od prvih tehnika koja se primijenjuje u rješavanju teških 
...
\begin{example}(\textbf{Problem razmjenjivanja novca}). 
 Neka je na raspolaganju  $X$ KM-ova. Na rapolaganju su apoeni od $x_1, x_2, \ldots , x_k$ KM-ova, negoraničene količine. Pretpostavimo da su $X, x_i, i=1,\ldots, n$ prirodni brojevi.
...
\begin{example} (\textbf{Razlomljeni problem ruksaka}). 
  Ovaj problem predstavlja relaksiranu verziju problema ruksaka, uveden u prethodnim sekcijama. Odluka da li se uzima (cio)  proizvod ili ne i stavlja u ruksak se relaksira na slučaj da se bilo koji (razlomljeni) dio tog proizoda može uzeti i staviti u ruksak. Zamislimo da je u pitanju proizvod kao što je šećer ili ulje koji (teorijski) mogu da se dijele na (realne) dijelove.  \\
...
\textit{Pohlepna funkcija}. Neka je $S$ parcijalno rješenje i neka ono pokriva $X_S \subseteq X$ elemenata. Intuitivno, bolje opcije podrazumijevaju biranje kompomenti rješenja $C$ koje u presjeku sa skupom $X \setminus X_S$ imaju više elemenata. Razlog je taj što nastojimo pokriti što više elemenata skupa $X$ u što manjem broju koraka pohlepnog algoritma. U simboličkom zapisu, pohlepna funkcija ima sljedeći oblik
...
maksimalni profit. (Šta predstavlja problem sa ovakvim pristupom?) Međutim, razmotrimo  tehniku pohlepnih algoritama.   \\ \vspace{0.2cm}
...
Da bismo realizovali pomenute procedure efikasno, sortirajmo poslove u odnosu na njihov profit (opadajuće). Bez smanjenja opštosti, pretpostavimo da su indeksi sortiranih poslova dati sa $\{1, 2, \ldots, n\}$. U prvoj iteraciji dodaje\-mo posao sa indeksom 1 u parcijalno rješenje $I_s$, koje je inicijalno prazno.  Dalje, pretpostavimo da je u $i$-toj iteraciji generisano rješenje $I_s$. Tada, u narednoj iteraciji razmatramo posao indeksiran sa $i+1$. Moguća su dva scenarija.
...
one podprobleme čija se rješenja kori\-ste vi\-še puta da bi se dobilo rješenje početnog problema. Pod optimalnim svojstvom podstrukture   podrazumijevamo proceduru kombinovanja optima\-lnih rješe\-nja podproblema u svrhu dobijanja  optimalnog  rješenja datog proble\-ma.   Ako se bilo koji problem može podijeliti na podprobleme, koji su zatim rekurzivno dijele na manje podprobleme, te ako postoji preklapanje među   njima, rješenja podproblema ima smisla  čuvati (u memoriji) za ponovnu upotrebu. Ovaj proces se naziva \textit{memoizacija}. Memoizacija poboljšava efikasnost rekurzivnog pristupa na taj način da sprečava ponovnu evaluaciju istog podproblema već se to rješenje  jednostavno pročita iz određene strukture podataka (poznata još kao heš struktura).   Međutim, ovaj pristup zahtjeva dodatnu memoriju  koja čuva rješenja podproblema. \\  
