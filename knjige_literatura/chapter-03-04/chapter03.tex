...
\begin{itemize}
    ...
    \item \textit{ Podijeli i vladaj} (eng. \textit{divide-and-conquer}) pristup uključuje podjelu problema na manje podprobleme, rješavanje svakog pod\-problema nezavisno, a zatim se rezultati  kombinuju da bi se riješio originalni problem. Dakle,  koristi se za efikasno rješavanje problema koji se mogu podijeliti na manje, nezavisne dijelove.
...
Rekurzija je moćna tehnika sa raznim primjenama u informatici i programiranju. Neke od uobičajenih primjena rekurzije uključuju sljedeće.

\begin{enumerate}
	\item  \textit{Pretraživanje stabla i grafa}. Rekurzija se često primjenjuje za   pretraživanje bitnih struktura podataka kao što su stabla i grafovi. Ovi  algoritmi se mogu koristiti za istraživanje svih čvorova stabla ili grafa na sistematičan način.
...
% https://www.geeksforgeeks.org/interpolation-search/  
 ...
 Interpolacijska pretraga je poboljšanje  binarnog pretraživanja   gdje pretposta\-vljamo da su vrijednosti u sortiranom nizu uniformno  raspo\-ređene. Interpolacija konstruiše nove tačke   unutar opsega diskretnog skupa strukture podataka. Kao što znamo, binarno pretraživanje uvijek polazi sa provjerom središnjeg elementa niza. Sa druge strane, interpolacijska pretraga  pretražuje na različitim mjestima u strukturi. Npr. ako je vrijednost traženog elementa  bliža posljednjem elementu, interpolacijska pretraga će vjerovatno započeti pretragu više prema krajnjoj strani strukture.
  
  Za pronalazak pozicije  \emph{pos} u nizu \emph{niz}, koristi se sljedeća ideja: pretražićemo više pozicije \emph{pos} ako je vrijednost elementa koji se traži bliži kraju niza, tj. $niz[n-1]$. Manja pozicija \emph{pos} se vraća kada je tražena vrijednost bliže početku niza, tj. elementu $niz[0]$. 
...
Ovu paradigmu možemo koristiti kada je potrebno vratiti rješenje razumnog kvaliteta   u relativno kratkom vremenskom intervalu, imple\-mentirajuči pristup relativno lako. Ovo je često jedna od prvih tehnika koja se primijenjuje u rješavanju teških 
...
\begin{example}(\textbf{Problem razmjenjivanja novca}). 
 Neka je na ra\-spolaganju  $X$ KM-ova i apoeni od $x_1, x_2, \ldots , x_k$ KM-ova, negoraničene količine. Pretpostavimo da su $X, x_i, i=1,\ldots, n$ prirodni brojevi.
...
\textit{Kompleksnost algoritma}. Sortiranje algoritma zahtjeva vrijeme od $O(n \log n)$. Dalje, glavna petlja se izvršava u $O(n)$ vremenu, odakle slijedi da je ukupno vrijeme izvršavanja algoritma jednako $O(n \log n)$, što je, svakako, polinomijalno.  Napomenimo da ovakvom strategijom garantujemo nalazak optimalnog rješenja problema, (ili utvrđujemo da rješenje ne postoji) što se formalno može i pokazati. 
...
\begin{solution}
    ...
    Komponente rješenje u odnosu na parcijalno rješenje $s$ su date svim onim proizvodima $C_s = [n] \setminus \{  {i_1}, \ldots, {i_k} \}$ koji se trenutno ne nalaze   u ruksaku. Proširenje parcijalnog rješenja podrazumijeva operaciju dodavanja  količine $c_r>0$ onog proizvoda $r \in C_s$, tako da proširivanjem rješenja $c$ ovom komponentom ne narušava kapacitet ruksaka. 
...
\textit{Pohlepna funkcija}. Neka je $S$ parcijalno rješenje i neka ono pokriva $X_S \subseteq X$ elemenata. Intuitivno, bolje opcije podrazumijevaju biranje kompomenti rješenja $C$ koje u presjeku sa skupom $X \setminus X_S$ imaju više elemenata. Razlog je taj što nastojimo pokriti što više elemenata skupa $X$ u što manjem broju koraka pohlepnog algoritma. U simboli\-čkom zapisu, pohlepna funkcija ima sljedeći oblik
...
\begin{example} (\textbf{\textit{Problem sekvencijalnog raspoređivanja poslova}}). Dat je skup  poslova, gdje svaki posao ima dozvoljeno krajnje
	...
	vrijeme. Startno vrijeme za izvršavanje svih poslova je isto za sve poslove (pretpostavljamo da kreće od 0). \\
	  ...
	  Odabrati podskup poslova koje možemo da izvršimo tako da se maksimizujemo ukupan profit. Poštovati  ograničenje da se maksimalno jedan
...
one podprobleme čija se rješenja kori\-ste vi\-še puta da bi se dobilo rješenje početnog problema. Pod optimalnim svojstvom podstrukture   podrazu\-mijevamo proceduru kombinovanja optima\-lnih rješe\-nja podproblema u svrhu dobijanja  optimalnog  rješenja datog proble\-ma.   Ako se bilo koji problem može podijeliti na podprobleme, koji su zatim rekurzivno dijele na manje podprobleme, te ako postoji preklapanje među   njima, rješenja podproblema ima smisla  čuvati (u memoriji) za ponovnu upo\-trebu. Ovaj proces se naziva \textit{memoizacija}. Memoizacija poboljšava efikasnost rekurzivnog pristupa na taj način da sprečava ponovnu evaluaciju istog podproblema već se to rješenje  jednostavno pročita iz određene strukture podataka (poznata još kao heš struktura).   Međutim, ovaj pristup zahtjeva dodatnu memoriju  koja čuva rješenja podproblema. \\  
...
Dinamičko programiranje se uglavnom primjenjuje na rekurzivne algori\-tme.  U  osnovi, velika većina problema optimizacije u rješavanju zahtijeva definisanje rekurzije, dok se za njenu  optimizaciju koristi dinamičko programiranje.

Ne mogu se svi problemi koji koriste rekurziju upotrijebiti dinamičko programiranje za svoje rješavanje. Osim ako ne postoji prisustvo podproblema koji se preklapaju,  kao u problemu računanja Fibonačijevog broja, rekurzija može doći do (jednako efikasnog) rješenja problema pomoću pristupa zavadi pa vladaj. U ovome nalazimo  razlog zašto rekurzivni algoritam koji realizuje  sortiranje spajanjem ne treba da koristi dinamičko programiranje, jer se podproblemi ni na koji način ne preklapaju (dijeljenje niza na dva disjunktna podniza).
...
\begin{solution}
	    ...
	    \begin{itemize}
	    	...
	    	\item  $s_1[i-1] \neq s_2[j-1]$: U ovom slučaju, sigurno karakter predstavljen komponentnim parom  $(s_1[i-1]$, $s_2[j-1])$ ne doprinosi optimalnom rješenju podproblema $(i,j)$. Dakle,  za određivanje optimuma podproble\-ma $(i,j)$ relevantna su dva podproblema $(i-1, j)$ i $(i, j-1)$, nastala izbacivanjem krajnjih karaktera prvo u prvom, pa onda u drugom prefiksu. Duže (optimalno) rješenje ta dva podproblema će činiti i optimalno rješenje podproblema $(i,j)$. Dakle, u ovom slučaju vrijedi rekurzija:
...
\begin{example}
	...
	Pronaći optimalan način rezanja  štapa na manje	štapove da bi se maksimizovao ukupan profit.
\end{example}
