...
\textit{Algoritmi pretraživanja} su dizajnirani sa zadatkom provjeravanja da li se objekat nalazi (ili da se vrati) u strukturi podataka u kojoj su  pohranjeni objekti; takođe,  da se vrati poruka ako takav element ne postoji.
...
Ideja binarne pretrage se sastoji u eksploatisanju svojstva sortiranog niza.  Bez smanjenja opštosti pretpostavimo da je niz sortiran u rastućem poretku. To znači da u slučaju da se na poziciji (indeksu) $i$ nalazi element veći od traženog elementa $x$, $x$ se (potencijalno) nalazi lijevo od indeksa $i$, te indekse $i+1, \ldots$ isključujemo dalje iz razmatranja. U svakoj iteraciji, biramo indeks $i$ kao središnji element niza, koji se potom poredi sa $x$. U zavisnosti od rezultata, imamo tri mogućnosti: ($i$) element na poziciji $i$ je upravo $x$, čime prekidamo pretragu vraćajući dati indeks; ($ii$)  element na poziciji $i$ je veći od $x$, čime pretragu usmjeravamo na elemente niza lijevo od pozicije $i$; ($iii$) inače, pretraga se seli na desni dio niza, tj. elemente čiji je indeks veći od $i$. Za bazne slučajeve uzimamo  niz veličine 0 ili 1. U prvoom slučaju vraćamo \textit{False}, dok u drugom slučaju vraćamo rezultat poređenja elementa niza sa $x$.  
...
Rekurzivna implementacija pretage preskakanjem  je data sljedećim kodom.  % u jeziku Pajton. 
