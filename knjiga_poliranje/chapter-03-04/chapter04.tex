...
\textrm{SPF}[0], \textrm{SPF}[1], \ldots, \textrm{SPF}[N]$ (eng. \emph{Smallest Prime Factor}) definisan tako da vrijednost $\textrm{SPF}[i]$ predstavlja najmanji prosti faktor broja $i$. Na primjer, $
\textrm{SPF}[12]=2,
\textrm{SPF}[15]=3,
\textrm{SPF}[49]=7.$ Za proste brojeve vrijedi $ \textrm{SPF}[p]=p. $

Nakon izgradnje niza $\textrm{SPF}$, faktorizacija broja $n$ postaje veoma jednostavna: u svakom koraku uzimamo vrijednost $\textrm{SPF}[n]$, dodajemo je u listu faktora i zamjenjujemo $n$ vrijednošću $n \leftarrow \frac{n}{\textrm{SPF}[n]}.$
...
Zbog toga je ovaj pristup naročito pogodan kada je potrebno faktorisati veliki broj različitih brojeva iz istog intervala. Implementacija ovakve (nizovne) strukture je data narednim k\^odom. 
...
pri čemu je $\binom{n}{0} := 1$. Binomni koeficijent $\binom{n}{1} = n,$ dok je $\binom{n}{n} = 1$ za sve $n \in \mathbb{N}.$ Takođe, ako je $n <k$, vrijedi $\binom{n}{k}=0$.   Lako se može pokazati da vrijedi Paskalova jednakost:
...
1, 1, 2, 5, 14,$ 42, 132, $429, 1430, 4862,\ldots$ Katalanovi brojevi zadovoljavaju sljedeću rekurzivnu relaciju: 
$$
C(0)=1, \text{ i } 
...
Optimizujmo ovaj naivni rekurzivni pristup za računanje Katalanovih brojeva. Kako je primijetno svaki podproblem izražunava iznova više puta, ponovo iskoristimo princip memoizacije. Čuvajmo vrijednost $i$-tog Katalano\-vog broja u nizovnoj strukturi $\textrm{Cat}(i)$.  Svojstvo optimalne podstrukture je dato rekurzijom $\textrm{Cat}(n+1) = \sum_{i=0}^{n} \textrm{Cat}(i) \cdot \textrm{Cat}(n-i).$ Bazni slučaj je dat sa $\textrm{Cat}(0) =1$.
...
$$
 ...
 \begin{solution}
 	...
 	Posmatrajmo primjer instance: $\texttt{coef}=[1,-3,2,-1], x=3.$ U ovom slučaju riječ je o polinomu $
...
\end{solution}
 ...
 \begin{enumerate}
 	...
 	\item Sfenički brojevi su pozitivni cijeli brojevi koji se dobijaju kao proizvod tačno 3 različita prosta faktora. Npr. $30, 42, 66, 70, 78,$ $102, 105, 110, 114, \ldots$ su neki od takvih brojeva. U ulazu je dat broj $n$. Ispitati da li je $n$ sfenički? 
