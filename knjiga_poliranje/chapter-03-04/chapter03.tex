...
\begin{solution}
	...
	Preostaje da se $n-1$ diskova sa štapa $B$ premjesti na štap $C$, za šta je ponovo potrebno $T(n-1)$ poteza.  	Prema tome, ukupan broj poteza zadovoljava rekurentnu relaciju $ T(n)=T(n-1)+1+T(n-1),$  odnosno	$	T(n)=2T(n-1)+1, 	T(1)=1.$ Ovo predstavlja rekurzivnu formulaciju problema Hanojskih kula.
...
\end{solution}	
 ...
 %SORTIRANJE ALGORITAMA: TODO 
\textbf{Rekurzivni algoritam sortiranja umetanjem}.  
  
 Sortiranje umetanjem (\emph{Insertion Sort}) predstavlja jednostavan algoritam
 sortiranja koji niz gradi postepeno, održavajući jedan njegov dio sortiranim.
 Osnovna ideja algoritma sastoji se u tome da se svaki novi element umetne na
 odgovarajuću poziciju unutar već sortiranog dijela niza.
 ...
 Na ovaj način nakon završetka postupka prvih $n$ elemenata niza nalazi se u
 sortiranom poretku.
...
\end{minted}
 ...
 \paragraph{Ispravnost algoritma.}
 Ispravnost rekurzivnog algoritma možemo pokazati indukcijom po veličini
 niza $n$.
 
 Za $n=1$ niz sadrži samo jedan element i trivijalno je sortiran.
 
 Pretpostavimo sada da algoritam ispravno sortira svaki niz od $n-1$
...
\begin{solution}
	...
	DP[n][C].$	Drugim riječima, ona predstavlja najveću moguću vrijednost koju možemo smjestiti u ruksak kapaciteta $C$ koristeći svih $n$ proizvoda.
...
\begin{solution}
	...
	DP[i][0]=True, 
	0\leq i\leq n.$ 	S druge strane, ako ne raspolažemo niti jednim elementom, nijedan pozitivan zbir ne može biti formiran $ DP[0][j]=False, 1\leq j\leq sum.
	$
	...
	def sum_array_top_down(niz, sum, i, j, DP = [ ]):
		...
		if j >= niz[i-1]:
			DP[i][j] = sum_array_top_down(niz, sum, i-1, j, DP) 
	                or sum_array_top_down(niz, sum, i-1, j-niz[i-1], DP)
			return DP[i][j]
		else:
			DP[i][j] = sum_array_top_down(niz, sum, i-1, j, DP)
			return DP[i][j]
...
\begin{solution}
	...
	\frac{n(n+1)}{2},
	$  pa je vremenska složenost algoritma $O(n^2).$  Prostorna složenost iznosi $O(n),$ jer se koristi jednodimenzionalni niz $DP$ veličine $n+1$.
