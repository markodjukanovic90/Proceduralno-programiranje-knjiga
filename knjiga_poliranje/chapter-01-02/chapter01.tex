...
Prva verzija jezika, Pajton 0.9.0, već tada sadrži brojne koncepte koji i danas predstavljaju osnovu jezika, uključujući funkcije, klase, izuzetke i module.
...
Pajton se posebno ističe jednostavnošću sintakse i čitljivošću programskog k\^oda. Za razliku od mnogih drugih programskih jezika, naglasak je stavljen na preglednost i jasnoću programa, zbog čega se često preporučuje kao prvi programski jezik za učenje programiranja. Istovremeno, njegova izražajnost i bogat skup biblioteka čine ga pogodnim i za razvoj složenih profesionalnih softverskih sistema.
...
\begin{itemize}
	...
	\item objasni osnovne faze izvršavanja programa napisanog u programskom jeziku Pajton, uključujući
...
\begin{itemize}
	\item \textit{Jezik visokog nivoa} (eng. \textit{high-level language}). Pajton omogućava programiranje na visokom nivou apstrakcije, pri čemu se programer može fokusirati na rješavanje problema, a ne na detalje rada računara. Sintaksa jezika je jednostavna i pregledna, zbog čega je k\^od relativno lako čitati i održavati.
	...
	\item \textit{Objektno-orijentisani jezik}. U Pajtonu su podaci i operacije nad njima organizovani kroz objekte i klase. Objektno-orijentisano programiranje omogućava modelovanje problema pomoću objekata i njihovih međusobnih interakcija, što često dovodi do preglednijeg i modularnijeg k\^oda.
...
Promjenljive u programskom jeziku Pajtonu kreiraju se dodjelom vrijednosti pomoću operatora dodjele \texttt{=}. Za razliku od mnogih drugih programskih jezika, nije potrebno unaprijed deklarisati tip promjenljive. Pajton automatski određuje odgovarajući tip na osnovu dodijeljene vrijednosti.
...
U narednim sekcijama detaljnije ćemo razmotriti numeričke, tekstualne i složene tipove podataka, kao i način njihovog korištenja u programima.
 ...
 Promjenljiva \texttt{x} referencira na objekat koji predstavlja broj 42, što se može ilustrovati kao na Slici~\ref{fig:var_object}.
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=170pt,height=120pt]{slike/vars_different_assigned.png}
 ...
 Napomenimo da ove oznake ne mijenjaju način rada jezika, već prvenstveno služe kao dokumentacija i pomoć alatima za statičku analizu k\^oda.
 ...
 Kada broj referenci nekog objekta postane jednak nuli, objekat više nije dostupan programu i njegova memorija može biti oslobođena. Pored toga, Pajton koristi i mehanizam sakupljanja otpadaka koji uklanja objekte koji više nisu potrebni, uključujući i složenije slučajeve kružnih referenci.
...
\section{Ispis poruka i komentari}
...
Tip \texttt{complex} koristi se za predstavljanje kompleksnih brojeva oblika  $a+bj,$ gdje su $a$ i $b$ realni brojevi, dok simbol \texttt{j} predstavlja imaginarnu jedinicu.  Primjer rada sa kompleksnim brojevima:
...
Realni i imaginarni dio kompleksnog broja mogu se dobiti pomoću atributa \texttt{real} i \texttt{imag}.  Kompleksni broj moguće je kreirati i korišćenjem funkcije \texttt{complex()}:
...
\subsection{\texttt{For} i \texttt{While} petlje}
...
\textit{\texttt{For} petlja.}  Koristi se za prolazak kroz elemente iterabilnih objekata, kao što su liste, torke, stringovi, skupovi i rječnici.
...
Ovaj k\^od ispisuje brojeve od 0 do 4.

\paragraph{\texttt{while} petlja.} \texttt{while} petlja izvršava blok koda sve dok je zadovoljen zadati logički uslov. Njena osnovna sintaksa je:
...
Prije svake iteracije provjerava se vrijednost izraza \texttt{uslov}. Ako je njegova vrijednost \emph{True}, izvršava se tijelo petlje. Kada uslov postane netačan (\emph{False}), izvršavanje petlje se završava, a program nastavlja sa prvom naredbom nakon petlje.
...
i koristi se za povećavanje vrijednosti promjenljive $i$  za jedan (inkrementiranje).

\section{Složene strukture podataka}

Pored osnovnih numeričkih tipova podataka, Pajton pruža i niz složenijih struktura podataka koje omogućavaju efikasno organizovanje i obradu većih količina informacija. Najvažnije među njima su:
...
Sve navedene strukture predstavljaju kolekcije objekata i koriste se u gotovo svakom ozbiljnijem programskom jeziku.
...
\paragraph{Inicijalizacija stringova.}  String se može definisati korišćenjem jednostrukih ili dvostrukih navodnika:
...
\paragraph{Korisne metode.} Klasa \texttt{str} sadrži veliki broj ugrađenih metoda. Neke od najčešće korištenih su:
...
%Njihovu detaljniju upotrebu prikazaćemo u narednim primjerima.\\
...
\textit{Dodatna napomena}. Pajton interno koristi različite optimizacije za rad sa stringovima. Jedna od njih je \textit{interniranje stringova} (eng. \textit{string interning}), pri čemu se identični stringovi po potrebi mogu dijeliti između više promjenljivih. Time se smanjuje potrošnja memorije i poboljšavaju performanse programa.

\paragraph{Formatiranje stringova.} Prilikom ispisa rezultata često je potrebno kombinovati tekst i vrijednosti promjenljivih. Pajton nudi nekoliko načina za formatiranje stringova.
...
Kod \texttt{f}-stringova vrijednosti promjenljivih navode se direktno unutar vitičastih zagrada, što k\^od čini preglednijim i jednostavnijim za pisanje.

Zbog svoje čitljivosti i efikasnosti, \texttt{f}-stringovi predstavljaju preporučeni način formatiranja stringova u savremenom Pajtonu.
...
Za razliku od mnogih drugih programskih jezika, elementi liste ne moraju biti istog tipa. Zbog toga kažemo da su liste \textit{heterogene} kolekcije. Na primjer, ista lista može sadržavati brojeve, stringove, logičke vrijednosti i druge objekte.  Elementi liste indeksiraju se brojevima počevši od nule. Pored standardnog indeksiranja, Pajton podržava i negativne indekse. Pri tome indeks \texttt{-1} označava posljednji element liste, \texttt{-2} pretposljednji itd.

\paragraph{Inicijalizacija liste.}  Liste se mogu kreirati na više načina: 
...
\paragraph{Pristup elementima.}  Pojedinačnim elementima liste pristupa se pomoću operatora \texttt{[]}.
...
\paragraph{Rezanje liste (eng. slicing).}
...
\texttt{Provjera pripadnosti}.  Operator \texttt{in} koristi se za provjeru da li se određeni element nalazi u listi.
...
Vrijednost izraza je \emph{True} ako se element nalazi u listi, odnosno \emph{False} u suprotnom.

\textit{Iteriranje kroz listu}. Elementima liste može se pristupati jedan po jedan pomoću \texttt{for} petlje.
...
\textit{Dodavanje i brisanje elemenata}. Novi element može se dodati na kraj liste metodom \texttt{append()}:
...
\textit{Umetanje elemenata}.  Element se može umetnuti na proizvoljnu poziciju korištenjem metode \texttt{insert()}:
...
\textit{Obrtanje i kopiranje liste}. Redoslijed elemenata liste može se obrnuti metodom \texttt{reverse()}:
...
\textit{Dodatne napomene}.   Za razliku od klasičnih nizova u programskom jeziku \texttt{C}, liste u Pajtonu mogu sadržavati elemente različitih tipova:
...
Pored lista, Pajton nudi i modul \texttt{array}, koji omogućava rad sa \textit{homogenim} nizovima podataka. Po svojoj organizaciji i načinu korištenja, objekti tipa \texttt{array} slični su nizovima u programskim jezicima {C} i {C++}.
...
\textit{Inicijalizacija}. Objekat tipa \texttt{array} kreira se navođenjem oznake tipa i početnih vrijednosti:
...
Ipak, u naučnom računarstvu i analizi podataka danas se mnogo češće koriste strukture iz biblioteke \texttt{NumPy}, koje predstavljaju proširenje ideje homogenih nizova i nude znatno bolje performanse i bogatiji skup funkcionalnosti.
...
\textit{Inicijalizacija}.  Rječnik se može kreirati na više načina.
...
\begin{itemize}
	...
	\item \textbf{Brisanje elementa}: Element se može ukloniti korištenjem ključne riječi \texttt{del} ili metode \texttt{pop()}.
	...
	\item \textbf{Provjera postojanja ključa}:   Operator \texttt{in} provjerava da li se određeni ključ nalazi u rječniku.
	...
	\item \textbf{Druge korisne metode}: 
...
\textit{Memorijska organizacija}. Rječnici su u Pajtonu implementirani korištenjem heš tabela. Osnovna ideja sastoji se u tome da se svaki ključ transformiše u cijeli broj primjenom heš funkcije. Dobijena vrijednost koristi se za brzo pronalaženje mjesta na kojem se nalazi odgovarajuća vrijednost.
...
\textit{Dodatne napomene}.  Ključevi u rječniku moraju biti nepromjenljivi objekti. Zbog toga se kao ključevi mogu koristiti objekti tipa \texttt{int}, \texttt{float}, \texttt{str} ili \texttt{tuple}, dok liste (\texttt{list}) ne mogu biti ključevi.
...
\textit{Inicijalizacija}.  Skup se može kreirati na nekoliko načina:
...
\end{minted}
 ...
 Prazan skup se kreira pomoću funkcije \texttt{set()}. Izraz
\begin{minted}{python}
	skup = {}
\end{minted}
...
\textit{Memorijska organizacija}.  Skupovi su u Pajtonu implementirani korištenjem heš tabela. Svakom elementu pridružuje se heš vrijednost koja omogućava veoma brzo pronalaženje njegove pozicije u memoriji.
...
\end{figure}
\textit{Dodatne napomene}.  Skupovi su naročito korisni kada je potrebno:
...
Torka (\texttt{tuple}) predstavlja uređenu, indeksiranu i nepromjenljivu  kolekciju elemenata. U Pajtonu se implementira pomoću ugrađenog tipa podataka \texttt{tuple}.
...
\textit{Inicijalizacija}. Torka se može kreirati na nekoliko načina:
...
\begin{itemize}
	...
	\item \textbf{Pristup elementima}:	Elementima torke pristupa se pomoću operatora \texttt{[]}:
	...
	\item \textbf{Broj pojavljivanja elementa} u torci se dobija metodom \texttt{count()}:
	...
	\item \textbf{Pronalaženje indeksa elementa} se vrši  metodom \texttt{index()}:
	
	\begin{minted}{python}
		print(my_tuple.index(2))  # 1
	\end{minted}
	
	\item \textbf{Dužina torke} se vraća funkcijom \texttt{len()}:
	\begin{minted}{python}
		print(len(my_tuple))  # 6
	\end{minted}
	
	\item \textbf{Spajanje torki}: Dvije torke mogu se spojiti operatorom \texttt{+}.
	...
	\item \textbf{Rezanje torke (slicing)}.	Kao i kod lista, moguće je izdvojiti dio torke
...
\textit{Memorijska organizacija}. Torke su nepromjenljive strukture podataka. Nakon kreiranja nije moguće dodavati, uklanjati niti mijenjati njihove elemente. Zbog toga Pajton može efikasnije upravljati memorijom nego kod promjenljivih struktura kao što su liste.
...
Pajton omogućava definisanje funkcija koje prihvataju proizvoljan broj pozicionih ili imenovanih argumenata.  Operator \texttt{*} sakuplja dodatne pozicione argumente u torku:
...
\textit{Prosljeđivanje argumenata}.  U Pajtonu se funkcijama prosljeđuje referenca na objekat. Zbog toga ponašanje zavisi od toga da li je objekat promjenljiv ili nepromjenljiv.
...
Pošto funkcija i pozivalac referenciraju isti objekat liste, promjena je vidljiva i nakon povratka iz funkcije. Ako želimo izbjeći izmjenu originalne liste, potrebno je funkciji proslijediti kopiju:
...
Cijeli brojevi, decimalni brojevi, stringovi i torke predstavljaju nepromjenljive objekte. Njihova izmjena u tijelu funkcije zapravo podrazumijeva kreiranje novog objekta.
...
\textit{Globalne varijable}. Ključna riječ \texttt{global} omogućava izmjenu pristupa varijabli unutar funkcije gdje ona postaje globalno vidljiva. 
...
\section{Neki programski koncepti i često korištene ugrađene funkcije}
...
Komprehenzija liste često rezultira kraćim i preglednijim k\^odom, posebno kod jednostavnih transformacija i filtriranja podataka.
...
Funkcija \texttt{map()} prihvata funkciju i iterabilni objekat te primjenjuje zadatu funkciju na svaki element kolekcije.
...
U Pajtonu verzije 3 funkcija \texttt{map()} vraća iterator, pa je potrebno rezultat pretvoriti u listu pozivom funkcije \texttt{list()}.

%\subsubsection*{Funkcija \texttt{filter()}}

Funkcija \texttt{filter()} služi za izdvajanje elemenata iterabilne kolekcije koji zadovoljavaju određeni uslov.
...
Funkcija koja se prosljeđuje kao prvi argument mora vraćati logičku vrijednost (\textit{True} ili \textit{False}).
...
U savremenom Pajtonu mnoge operacije realizovane funkcijama \texttt{map()} i \texttt{filter()} često se zapisuju pomoću komprehenzije liste, jer je takav k\^od pregledniji. Na primjer:
...
\begin{itemize}
	\item \textit{False} -- rastući poredak (podrazumijevano);
	\item \textit{True} -- opadajući poredak.
\end{itemize}
...
\subsection{Korisne ugrađene funkcije}

Pored navedenih funkcija, Pajton sadrži veliki broj korisnih ugrađenih funkcija. Neke od najčešće korištenih su:

\begin{itemize}
	\item \texttt{sum}(\emph{iterable}) -- vraća sumu elemenata iterabilnog objekta;
	\item \texttt{max}(\emph{iterable}) -- vraća najveći element;
	\item \texttt{min}(\emph{iterable}) -- vraća najmanji element;
	\item \texttt{abs}($x$) -- vraća apsolutnu vrijednost broja;
	\item \texttt{round}($x$, $n$) -- zaokružuje broj na \texttt{n} decimalnih mjesta;
	\item \texttt{len}(\emph{obj}) -- vraća broj elemenata kolekcije;
	\item \texttt{eval}(\emph{expr}) -- izračunava izraz zapisan u obliku stringa.
\end{itemize}

\section{Moduli}

\textit{Modul} predstavlja datoteku koja sadrži definicije funkcija, klasa, promjenljivih i drugih programskih elemenata. Modul sadrži jednu ili više datoteka sa ekstenzijom \texttt{.py} %Naziv modula odgovara nazivu datoteke sa ekstenzijom \texttt{.py}.

Moduli služe organizovanju programa u manje cjeline i ponovnu upotrebu k\^oda u različitim projektima. Da bismo ga koristili, modul je potrebno prvo uključiti pomoću naredbe \texttt{import}.

Na primjer, ako postoji datoteka \texttt{mojmodul.py},  uključuje na sljedeći način:

\begin{minted}{python}
	import mojmodul
\end{minted}

Nakon toga funkcijama, klasama i promjenljivama modula pristupamo pomoću notacije tačka:
...
Moduli koji su dio standardne biblioteke dostupni su odmah nakon instalacije Pajtona i nije ih potrebno dodatno instalirati. Sa druge strane, eksterne module je potrebno prethodno instalirati.
...
Nakon pokretanja naredbe, \texttt{pip} automatski preuzima traženi paket sa repozitorijuma PyPI (eng. \emph{Python Package Index}) i instalira ga u trenutno Pajton okruženje.
...
Savremeni projekti napisani u Pajtonu često koriste virtuelna okruženja (eng. \emph{virtual environments}) kako bi se zavisnosti različitih projekata držale odvojeno. O ovom konceptu biće više riječi u kasnijim poglavljima.
...
Modul \texttt{copy} omogućava pravljenje kopija objekata. Posebno je koristan pri radu sa složenim strukturama podataka, kao što su liste, rječnici i ugniježđene kolekcije. Ovaj modul podržava dvije osnovne vrste kopiranja:
...
Funkcija \texttt{deepcopy()} rekurzivno kopira sve objekte sadržane unutar originalnog objekta, čime nastaje potpuno nezavisna kopija. Naziva se još kopiranje po vrijednosti. 
...
Tokom izvršavanja programa mogu se pojaviti različite greške koje nije moguće otkriti u fazi prevođenja programa. Takve greške nazivaju se \textit{izuzeci} (eng. \textit{exceptions}). Tipični primjeri izuzetaka su:
...
U ovom slučaju program neće biti momentalno prekinut, već će se izvršiti odgovarajuća poruka o grešci.
...
Još jedan koristan mehanizam predstavlja naredba \texttt{assert}, koja služi za provjeru da li određeni uslov važi.  Njena sintaksa je:
\begin{minted}{python}
...
Pravilna upotreba izuzetaka predstavlja važan dio razvoja pouzdanih programa. Obradom izuzetaka moguće je spriječiti neočekivani prekid izvršavanja, korisniku pružiti jasne informacije o nastaloj greški i povećati robusnost softvera.

\section{Rad sa datotekama}

Pomoću svojih ugrađenih funkcija, Pajton omogućuje jednostavan rad sa ulaznim i izlaznim datotekama. U toku rada, ovaj modul interaguje sa operativnim sistemom. 
U nastavku dajemo pregled najčešće korište\-nih metoda.

\textit{Čitanje iz datoteke}. Koristi se funkcija \texttt{open}, koja prihvata dva argu\-menta -- naziv datoteke (ili putanja do datoteke) i način u kojem se otvara datoteka (npr. za čitanje, pisanje, dodavanje, itd.). Pogledajmo sljedeći primjer: 
\begin{minted}{python}
...
Dakle, prvo otvaramo datoteku  \texttt{datoteka.txt} samo za čitanje. Zatim čitamo sadržaj datoteke koristeći metodu \texttt{read} i čuvamo ga u varijabli \textit{sadrzaj}. Konačno, zatvaramo datoteku pomoću metode \texttt{close} i ispisujemo sadržaj datoteke. 



\textit{Pisanje u fajl}. Za pisanje u datoteku koristimo funkciju \texttt{open}, koja otvara fajl sa određenim nazivom, ali ovaj put sa načinom ``\textit{w}" (eng. \texttt{write}), čime se dopušta pisanje u fajl. Pogledajmo sljedeći primjer.
\begin{minted}{python}
...
\textit{Dodavanje sadržaja u fajl}. Da biste dodali sadržaj u postojeći sadržaj datoteke, otvaramo fajl sa načinom ``\textit{a}" (eng. \textit{append}), a potom pomoću funkcije \texttt{write} upisujemo novi sadržaj na postojeći. 

Napomenimo da je dobra praksa zatvoriti datoteku nakon što zavr\-šite rad sa njom, kako bismo izbjegli bilo kakve potencijalne probleme sa oštećenjem datoteke ili gubitkom podataka. Praksa je da se rad u fajlovima odvija pod \texttt{try}-\texttt{except} blokom. Klase izuzetaka koje se u tim slučajevima mogu koristiti su: \texttt{FileNotFoundError} i \texttt{OSError}. 

% Upis json, csv, ispis itd. 

\subsection{Modul \texttt{json}}

Modul \texttt{json} se koristi  za enkodiranje i dekodiranje podataka sa i u JSON (eng. \textit{Java Script Object Notation}) formatu. JSON je  široko raspro\-stranjen format za razmjenu podataka, osobama  jednostavan za čitanje i pisanje, a mašinama za raščlanjivanje i generisanje.\\
...
\begin{itemize}
	...
	\item json.\textit{loads}() -- koristi se za dekodiranje JSON niza u strukturu podataka (obično heš mapa).
\end{itemize}
...
Regularni izraz (eng. \textit{regular expression}, skraćeno \textit{regex}) predstavlja šablon za opisivanje i pretraživanje tekstualnih nizova. Drugim riječima, regularni izraz definiše skup stringova koji zadovoljavaju određena sintaksna pravila, opisujući skup riječi koje se uklapaju u isti šablon. 
...
\begin{itemize}
	...
	\item \textbf{Kvantifikatori}. Kvantifikatori određuju koliko puta se izraz  može pojaviti  koji im neposredno prethodi.
...
U nastavku slijedi nekoliko primjera regularnih izraza.
...
\subsection{Modul \texttt{re}}

Modul \texttt{re} je ugrađeni Pajtonov modul namijenjen radu sa regularnim izrazima. Omogućava definisanje, pretraživanje i obradu tekstualnih obrazaca (eng. \textit{pattern}), pa predstavlja jedan od najvažnijih alata za manipulaciju tekstualnim podacima.
...
\begin{itemize}
	...
	Pretražuje string \texttt{string} i pronalazi prvo pojavljivanje obrasca \texttt{pattern}. Ukoliko se obrazac pronađe, funkcija vraća objekat podudaranja; u suprotnom vraća vrijednost \texttt{None}.
...
koji predstavlja tzv. ravni string (eng. \textit{raw string}). Kod ovakvog zapisa znak \texttt{\textbackslash} se ne tretira kao početak specijalne sekvence, što pojednostavljuje pisanje regularnih izraza i smanjuje mogućnost greške.

\section{Moduli \texttt{sys} i \texttt{getopt}}
...
\paragraph{Modul \texttt{sys}}. 

Ovaj modul omogućava pristup informacijama i funkcijama vezanim za Pajtonov interpreter i izvršno okruženje. Neki od najčešće korištenih funkcija i atributa ovog modula su dati u nastavku.

\begin{itemize}
	
	\item \texttt{sys.argv}:  Lista argumenata komandne linije. Element na poziciji \texttt{0} sadrži naziv skripte, dok ostali elementi predstavljaju argumente koje je korisnik proslijedio programu.
	
	\item \texttt{sys.path}: Lista direktorijuma u kojima Pajton traži module za učitavanje.
	
	\item \texttt{sys.exit([kod])}:  Prekida izvršavanje programa i vraća izlazni kod operativnom sistemu. Ukoliko se kod ne navede, podrazumijeva se vrijednost 0.
	
	\item \texttt{sys.version}: Vraća informaciju o verziji Pajtonovog interpretera.
	
	\item \texttt{sys.platform}: Vraća oznaku platforme na kojoj se program izvršava.
...
\paragraph{Modul \texttt{getopt}}.   Ovaj modul služi za parsiranje opcija i argumenata komandne linije. Njegova funkcionalnost slična je istoimenoj funkciji dostupnoj u Unix operativnim sistemima. Najvažnija funkcija modula je:
...
\begin{itemize}
	...
	\item \textit{Kopiranje objekta}:
	\begin{minted}{python}
		kopija = arr.copy()
	\end{minted}
	
	\item \textit{Promjena oblika}:
	\begin{minted}{python}
		arr.reshape((2, 2))
	\end{minted}
	...
	\item \textit{Spajanje nizova}:
	\begin{minted}{python}
		np.concatenate((arr1, arr2), axis=0)
	\end{minted}
	
	\item \textit{Dijeljenje niza}:
	\begin{minted}{python}
		np.array_split(arr, 3)
	\end{minted}
	
	\item \textit{Pretraga elemenata}:
	\begin{minted}{python}
		np.where(arr % 2 == 0)
	\end{minted}
	...
	\item \textit{Sortiranje}:
	\begin{minted}{python}
		np.sort(arr)
	\end{minted}
	
	\item \textit{Filtriranje}:
	...
	\item \textit{Kreiranje specijalnih nizova}:
...
\paragraph{Memorijska organizacija.} Objekti biblioteke \texttt{NumPy} implementirani su klasom \texttt{ndarray}. Pored samih podataka, svaki objekat sadrži i određene metapodatke:
...
\section{Pisanje korisničkih modula}
...
Prije svega, potrebno je kreirati korisnički nalog na zvaničnoj PyPI stranici \url{https://pypi.org/account/register/}. 
...
\textit{Priprema paketa.} Pretpostavimo da je napisan k\^od u okviru paketa pod nazivom \texttt{modul1}. U korijenskom direktorijumu projekta kreiramo datoteku \texttt{pyproject.toml} koja sadrži osnovne metapodatke o paketu:
...
\textit{Kreiranje distribucije.} Distribucione datoteke kreiraju se naredbom:
...
Prije objavljivanja modula preporučuje se provjera ispravnosti generisanih datoteka:
...
Nakon instalacije paket se može koristiti u bilo kojem programu napisanim u Pajtonu:
\begin{minted}{python}
	import modul1
\end{minted}
...
Tokom izvršavanja programa operativni sistem i Pajtonov interpreter koriste različite dijelove memorije. Među njima posebno su važni \textit{stek} (eng. \textit{stack}) i \textit{hip}.

\textit{Stek} predstavlja memorijski prostor koji se koristi za čuvanje informacija o aktivnim pozivima funkcija. Svaki put kada se pozove funkcija, na steku se kreira novi zapis koji sadrži informacije potrebne za izvršavanje te funkcije.
...
Međutim, stvarni objekti na koje reference pokazuju uglavnom se nalaze u drugom dijelu memorije koji se naziva \textit{hip}.
...
Pajton koristi mehanizam poznat pod nazivom sakupljanje otpadaka (eng. \textit{garbage collection}). Njegov zadatak je automatsko uklanjanje objekata koji više nisu dostupni nijednom dijelu programa.
...
Zahvaljujući ovim mehanizmima, programer u većini slučajeva ne mora voditi računa o ručnom oslobađanju memorije, što značajno pojednostavljuje razvoj softvera i smanjuje mogućnost grešaka u vezi sa memorijom.

\subsection{Proces prevođenja i izvršavanja programa}

Iako se Pajton često opisuje kao interpretirani programski jezik, proces izvršavanja programa zapravo uključuje i fazu prevođenja izvornog k\^oda. Zbog toga se može reći da je Pajton istovremeno i kompajlirani i interpretirani jezik.
...
\begin{enumerate}
	...
	Na osnovu AST strukture Pajton generiše \textit{bajtkod} (eng. \textit{bytecode}), koji predstavlja međureprezentaciju nezavisnu od operativnog sistema i procesorske arhitekture.
	
	Generisani bajtkod može biti sačuvan u datotekama sa ekstenzijom \texttt{.pyc}.
	
	\item \textbf{Izvršavanje bajtkoda}
	
	Bajtkod izvršava Pajtonova virtuelna mašina (eng. \textit{Python Virtual Machine -- PVM}). Virtuelna mašina čita instrukcije bajtkoda i izvršava ih jednu po jednu, upravljajući memorijom, objektima i tokom programa.
...
Najvažnije funkcije PVM-a su navedene u nastavku. 

\begin{itemize}
	
	\item \textbf{Prenosivost programa}: Bajtkod je nezavisan od konkretne hardverske platforme. Zbog toga se isti Pajton program može izvršavati na različitim operativnim sistemima bez izmjene izvornog koda.
	
	\item \textbf{Izvršavanje bajtkoda}: PVM interpretira instrukcije bajtkoda i prevodi ih u odgovarajuće operacije nad objektima i memorijom.
	
	\item \textbf{Dinamičko tipiziranje}:  Pošto se tipovi objekata određuju tokom izvršavanja programa, PVM upravlja svim provjerama i operacijama vezanim za tipove podataka.
	
	\item \textbf{Upravljanje memorijom}: PVM koristi mehanizme brojanja referenci i sakupljanja otpadaka kako bi automatski oslobađala memoriju koja više nije potrebna.
	
	\item \textbf{Podrška za module i biblioteke}:  PVM omogućava učitavanje i korištenje velikog broja ugrađenih i eksternih biblioteka pomoću naredbe \texttt{import}.
...
Ukratko, Pajton prvo prevodi izvorni kôd u bajtkod, a zatim taj bajtkod izvršava pomoću virtuelne mašine. Zbog toga se često kaže da je Pajton hibrid između kompajliranih i interpretiranih programskih jezika.
